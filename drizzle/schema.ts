...
/**
 * RAG Vector Store - Website Content Chunks
 */
export const websiteChunks = pgTable("website_chunks", {
  id: varchar("id", { length: 64 }).primaryKey(),
  tenantId: varchar("tenantId", { length: 64 }).notNull().references(() => tenants.id, { onDelete: "cascade" }),
  ...
  embedding: text("embedding").notNull(), // Will store vector as JSON array
  metadata: text("metadata"), // JSON: {section, lastScraped, etc}
  createdAt: timestamp("createdAt").defaultNow(),
  updatedAt: timestamp("updatedAt").defaultNow(),
});

export type WebsiteChunk = typeof websiteChunks.$inferSelect;
export type InsertWebsiteChunk = typeof websiteChunks.$inferInsert;
...
export type ScrapingLog = typeof scrapingLog.$inferSelect;
export type InsertScrapingLog = typeof scrapingLog.$inferInsert;
