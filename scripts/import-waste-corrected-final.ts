...
async function importWasteSchedule() {
  try {
    console.log('🗑️ Importing CORRECTED FINAL waste schedule for November-December 2025...');
    
    // Load JSON file
    const jsonPath = path.join(__dirname, '..', 'waste_schedule_nov_dec_2025_corrected_final.json');
    ...
    for (const [ortsteilName, dates] of Object.entries(schedule.gruenschnitt_ortsteile)) {
      if (ortsteilName === 'comment') continue;
