...
async function scrapeHornBadMeinbergStaff() {
  console.log('Starting Horn-Bad Meinberg staff scraper (final version)...');
  console.log('');
  ...
  for (let page = 0; page < 4; page++) {
    ...
    try {
      ...
      $('h3.list-title').each((index, element) => {
        ...
        // Scrape detail page for phone and room
        const promise = (async () => {
          ...
          if (detailUrl) {
            const details = await scrapeDetailPage(detailUrl);
            phone = details.phone;
            room = details.room;
            await delay(200); // Rate limiting
          } else {
            console.log(`    Warning: No detail URL found`);
          }
